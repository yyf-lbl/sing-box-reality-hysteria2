...
# download singbox 
download_singbox() {
    echo -e "\e[1;3;33m正在下载 sing-box 内核...\e[0m"
    sleep 1
    arch=$(uname -m)
    ...
    case ${arch} in
        ...
        armv7l)
            ...
            ;;
        *)
            echo -e "\e[1;3;31m不支持的系统架构: $arch\e[0m"
            return 1
            ;;
    esac

    # 指定版本号
    specified_version="1.11.0-alpha.29"
    echo -e "\e[1;3;32m指定版本: $specified_version\e[0m"

    # 文件路径与名称
    package_name="sing-box-${specified_version}-linux-${arch}"
    ...
    # 下载文件
    url="https://github.com/SagerNet/sing-box/releases/download/v${specified_version}/${package_name}.tar.gz"
    if ! curl -Lo "$download_path" "$url"; then
        echo -e "\e[1;3;31m下载失败，请检查网络连接。\e[0m"
        return 1
    fi

    # 解压文件
    if ! tar -xzf "$download_path" -C /root; then
        echo -e "\e[1;3;31m解压失败，请检查下载文件是否损坏。\e[0m"
        rm -f "$download_path"
        return 1
    fi
    ...
    echo -e "\e[1;3;32m下载和配置完成！版本: $specified_version\e[0m"
}
